...
```
docker run -d --net=host --rm \
    -v $(pwd)/prometheus0_veggie.yml:/etc/prometheus/prometheus.yml \
...
```
docker run -d --net=host --rm \
    -v $(pwd)/prometheus1_veggie.yml:/etc/prometheus/prometheus.yml \
...
![](/root/no-read-tenancy-tomato.png)
...
![](/root/no-read-tenancy-reuse.png)
